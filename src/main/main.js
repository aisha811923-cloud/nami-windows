...
function claudeExec(argv) {
  return new Promise((resolve) => {
    ...
    execFile(bin, argv, { timeout: 20000, env: buildChildEnv({ settings: readSettings(), purpose: 'agent', agentId: 'claude' }), shell: process.platform === 'win32' }, (err) => {
...
ipcMain.handle('services:disconnect', async (_e, { id, projectPath }) => {
  ...
  const viaCli = validServiceId(id) ? await new Promise((resolve) => {
    const bin = knownBin('claude') || 'claude';
    execFile(bin, ['mcp', 'remove', '--scope', 'user', id], { timeout: 20000, env: buildChildEnv({ settings: readSettings(), purpose: 'agent', agentId: 'claude' }), shell: process.platform === 'win32' }, (err) => resolve(!err));
  }) : false;
...
ipcMain.handle('term:create', async (e, { id, cwd, cols, rows, kind, command, program, args, seed, cont, sid, acpSid, name, watchDone, oneShot, purpose, agentId }) => {
  ...
  if (kind === 'claude') {
    ...
    // this tile carries claude's keys, so it must not outlive claude.
      ...
      file = shellPath;
      const quote = (a) => shellQuote(a, process.platform);
      ...
      if (policy.purpose === 'agent' || promptArgs.length) {
        spawnArgs = isWin ? ['-NoLogo', '-Command', line] : ['-i', '-c', line];
        echoLine = displayLine;
      }
