...
async function deleteItem({ filePath, projectPath, homeDir, trashFn, existsFn = entryExists }) {
  const home = homeDir || os.homedir();
  const p = (typeof filePath === 'string' && filePath.startsWith('/')) ? path.posix : path;
  ...
  const inRoot = roots.some((r) => abs.startsWith(r + p.sep));
  const inPluginCache = abs.includes(p.sep + p.join('.claude', 'plugins') + p.sep);
