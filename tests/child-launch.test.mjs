...
test('an agent tile exits with the agent, and the shell still reads its rc file', async (t) => {
  if (process.platform === 'win32') { t.skip('POSIX zsh rc test'); return; }
...
test('the actual shell preserves every prompt character without interpreting it', async (t) => {
  if (process.platform === 'win32') { t.skip('POSIX /bin/sh test'); return; }
  const pty = recordingPty();
