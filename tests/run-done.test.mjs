...
test('a one-shot on powershell is structured correctly', () => {
  const args = oneShotArgs('powershell.exe', 'echo BEFORE; echo AFTER');
  assert.equal(args[0], '-NoExit');
  assert.equal(args[1], '-Command');
  assert.match(args[2], /echo BEFORE; echo AFTER/);
  assert.match(args[2], /NamiRunDone/);
});

test('spawned for real: powershell reports exit code and output survives', { skip: process.platform !== 'win32' }, () => {
  const args = oneShotArgs('powershell.exe', 'Write-Output BEFORE; Write-Output AFTER');
  const out = execFileSync('powershell.exe', ['-NoProfile', '-Command', args[2]], { encoding: 'utf8' });
  assert.equal(feedRunDone({}, out), 0);
  assert.match(out, /BEFORE/);
  assert.match(out, /AFTER/);
});
